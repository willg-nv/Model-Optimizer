...
class TestResolveRegionIOInsertionPoints(unittest.TestCase):
    ...
    def test_resolve_with_region(self):
        ...
        graph.tensor_users_map = {"conv_out": [1]}

        region = Region(region_id=1, level=0, region_type=RegionType.LEAF)
        region.add_node(1)  # BatchNorm node

        result = resolve_region_io_insertion_points(region, graph, "conv_out")

        assert len(result) >= 1
        assert any(ip.tensor_name == "conv_out" for ip in result)
    ...
    def test_resolve_without_region(self):
        ...
        graph.tensor_users_map = {"bn_out": [2]}

        result = resolve_region_io_insertion_points(None, graph, "bn_out")

        assert len(result) == 1
        ip = next(iter(result))
        assert ip.tensor_name == "bn_out"
...
class TestChildRegionInputInsertionPointResolve(unittest.TestCase):
    ...
    def test_resolve_multiple_children(self):
        ...
        graph.tensor_users_map = {"input": [0, 3], "conv1_out": [1]}
        ...
        # Second child: Relu1 (consumes "conv1_out")
        child2 = Region(region_id=3, level=0, region_type=RegionType.LEAF)
        child2.inputs = ["conv1_out"]
        child2.add_node(1)  # Relu1
        ...
        # Resolve input of second child (region_index=1) - "conv1_out" tensor
        ip2 = ChildRegionInputInsertionPoint(region_index=1, input_index=0)
        result2 = ip2.resolve(parent, graph)

        assert len(result2) >= 1
        assert any(rip.tensor_name == "conv1_out" for rip in result2)
...
class TestRegionOutputInsertionPointResolve(unittest.TestCase):
    ...
    def test_resolve_node_output(self):
        ...
        region.add_node(1)  # BatchNorm
        region.add_node(2)  # Relu
        region.outputs = ["conv_out"]

        # Output of first node (Conv)
        ip = RegionOutputInsertionPoint(region_index=None, node_index=0, output_index=0)

        result = ip.resolve(region, graph)

        assert len(result) >= 1
        assert any(rip.tensor_name == "conv_out" for rip in result)
