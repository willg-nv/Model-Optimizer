...
class RegionPattern:
    ...
    signature string. They are not considered equal if they have different sizes.
    ...
    def _compute_signature_recursive(region: Region, graph: gs.Graph) -> str:
        ...
        # Collect node signatures in sorted order
        node_ops = [
        ...
        # Get sorted children (by -level, size) for deterministic signatures
        # CRITICAL: This order MUST match insertion_points.py for pattern-relative indexing
        sorted_children = region.get_children(sort=True)
        ...
        child_sigs = RegionPattern._join_signatures(
            [RegionPattern._compute_signature_recursive(child, graph) for child in sorted_children]
        )

        if node_ops:
            return f"COMPOSITE({RegionPattern._make_node_signature(node_ops)}|{child_sigs})"
    ...
    def _get_symmetric_input_signature(
        ...
        For commutative operations (Add, Mul, etc.), sorts input sources to ensure
        consistent signatures regardless of operand order.
        ...
        Returns:
            Sorted input source signature string, or None if not a symmetric op.
            Example: "external:Conv,internal:Mul"
        """
