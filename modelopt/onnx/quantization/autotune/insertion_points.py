...
def has_quantizable_operations(region: "Region", graph: gs.Graph) -> bool:
    ...
    if region.get_type() == RegionType.LEAF:
        ...
        return bool(region_ops.intersection(get_autotuner_quantizable_ops()))
    return True
    ...
    for node_idx in node_indices:
        ...
        for input_idx, inp in enumerate(node.inputs):
            ...
            if inp.name == tensor_name:
                if skip_invalid_insertion_points(graph, tensor_name, node):
                    continue
                ip = ResolvedInsertionPoint(
...
def get_autotuner_skip_ops():
    ...
    """
    # Start with copy ops (reshape, gather, scatter, etc.)
    skip_ops = set(get_copy_ops())

    # Add additional ops not covered by get_copy_ops
    extra_indexing_ops = {"Compress"}
    extra_scatter_ops = {"Scatter"}
    extra_reshape_ops = {"ExpandDims", "Unsqueeze", "View"}
    extra_rearrangement_ops = {"Pad"}
