...
class Region:
    ...
    **Pattern Matching:**
    ...
    def get_children(self, *, sort: bool = False) -> list["Region"]:
        ...
        Args:
            sort: If True, return children sorted by (-level, size_of_region_and_descendants).
                  This ordering ensures deterministic pattern matching. Defaults to False.

        Returns:
            List of child regions, optionally sorted.
        """
        if not sort:
            return self.children
        return sorted(
            self.children, key=lambda r: (-r.get_level(), r.get_size_of_region_and_descendants())
        )
    ...
    def remove_child(self, child: "Region") -> bool:
        ...
        try:
            ...
            if child.parent and child.parent.get_id() == self.id:
    ...
    def get_nodes(self, *, sort: bool = False) -> list[int]:
        ...
        in child regions. Use get_region_nodes_and_descendants() for complete coverage.

        Args:
            sort: If True, return nodes in sorted order. Defaults to False.

        Returns:
            List of node indices (absolute positions in the graph).
        """
        if sort:
    ...
    def contains_node_within_region_and_descendants(
        self, node_index: int, _visited: set[int] | None = None
    ) -> bool:
        ...
        if self.contains_node(node_index):
            return True
        return any(
            child.contains_node_within_region_and_descendants(node_index, _visited)
            for child in self.children
        )
